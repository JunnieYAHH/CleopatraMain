...
export default function ProductSalesChart({ data }) {
    ...
    const productSales = async () => {
        try {
            ...
            const { data } = await axios.get(`${process.env.REACT_APP_API}/api/v1/productSales`, config)
    ...
    return (
        <div style={{ width: '100%', height: '200px' }}>
            <ResponsiveContainer style={{ backgroundColor: 'gray' }}>
                <PieChart>
                    <Pie
                        dataKey="percent"
                        nameKey="name"
                        ...
                        fill="#FF5733" // Change this color to your desired color
